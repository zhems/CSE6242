...
"""
def author():                                                                                             
        return 'zchia3' # replace sroutray3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903513617 # replace with your GT ID number      
...
#############################################
            ...
            #     self.node_degree[target] = self.node_degree[target] + 1
            # else:
...
#############################################
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file): #for each edge

### Implement your code here
#############################################
                new_scores[target] = new_scores[target] + damping_factor*scores[source]/self.node_degree[source]

            for i in range(self.max_node_id + 1):
                new_scores[i] = new_scores[i] + (1-damping_factor)*node_weights[i]
            scores = new_scores
#############################################
